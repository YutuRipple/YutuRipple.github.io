...
if (m == 12 && dd == 13) {
    document.getElementsByTagName("html")[0].setAttribute("style", "filter: grayscale(60%);");
    if (sessionStorage.getItem("isPopupWindow") != "1") {
        Swal.fire("今天是南京大屠杀" + (y - 1937).toString() + "周年悼念日\n🪔愿逝者安息😔");
        sessionStorage.setItem("isPopupWindow", "1");
    }
}
if (m == 8 && dd == 14) {
...
if (m == 10 && dd <= 3) {//国庆节
    if (sessionStorage.getItem("isPopupWindow") != "1") {
        Swal.fire("祝中华人民共和国" + (y - 1949).toString() + "岁生日快乐！");
        sessionStorage.setItem("isPopupWindow", "1");
    }
}
if (m == 10 && dd == 10) {//国庆节
    if (sessionStorage.getItem("isPopupWindow") != "1") {
        Swal.fire("祝中华民国" + (y - 1911).toString() + "岁生日快乐！");
        sessionStorage.setItem("isPopupWindow", "1");
    }
}
if (m == 8 && dd == 15) {
    if (sessionStorage.getItem("isPopupWindow") != "1") {
        Swal.fire("对日反侵略战争胜利纪念日" + (y - 1945).toString() + "年了🎉");
        sessionStorage.setItem("isPopupWindow", "1");
    }
}
if (m == 1 && dd == 1) {//元旦节
...
l = ["非常抱歉，因为不可控原因，博客将于明天停止运营！", "好消息，你的天堂终身游申请通过了！", "美国垮了，原因竟然是克鲁鲁公主玩滑板！", "微软垮了！", "你的电脑已经过载，建议立即关机！", "你知道吗？站长很喜欢你哦！", "一分钟有61秒哦", "你喜欢的人跟别人跑了！"]
...
if (m == 7 && dd == 4) {//生日
    if (sessionStorage.getItem("isPopupWindow") != "1") {
        Swal.fire("祝愚図涟站长" + (y - 2002).toString() + "岁生日快乐！");
        sessionStorage.setItem("isPopupWindow", "1");
    }
}
if (m == 9 && dd == 10) {//教师节
